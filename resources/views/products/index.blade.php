                ...
                @if (count($products) > 0)
                    @foreach ($products as $product)
                        ...
                        <div style="display: flex; align-items: center;">     
                            <a href="{{ route('products.edit', $product->id) }}" class="btn-link btn btn-success" style="font-size: 16px;">
                                <i class="fas fa-pencil-alt" ></i> 
                            </a>
                            <form method="post" action="{{ route('products.destroy', $product->id) }}" >
                                @method("delete")
                                @csrf
                                    <button class="btn btn-danger" onclick="deleteConfirm(event)" style="font-size: 16px;">
